...
char* get_ip_addr_from_custom_data(char* custom_data)
{
    ...
    if (root==NULL)
    {
        LOG_E("%s is't json data", cjson_root);
        cJSON_Delete(root);
        return NULL;
    }
    ...
    if (add_type==NULL)
    {
        LOG_E("%s not \"mqtt\" project ", cjson_root);
        cJSON_Delete(root);
        return NULL;
    }
    ...
    if (addr==NULL)
    {
        LOG_E("%s not \"addr\" project ", cjson_root);
        cJSON_Delete(root);
        return NULL;
    }
...
static void example_event_callback(_blufi_cb_event_t event, _blufi_cb_param_t* param)
{
    ...
    switch (event)
    {
        case AXK_BLUFI_EVENT_INIT_FINISH:
        {
            ...
            LOG_I("BLUFI init finish,ble name=%s\r\n", g_blufi_config.ble.blufi.blufiname);
            axk_blufi_adv_start();
            blufi_is_start = true;
        }
        break;
        ...
        case AXK_BLUFI_EVENT_SET_WIFI_OPMODE:
            ...
            // {
            //    LOG_I("BLUFI axk_hal_wifi_mode_set fail\r");
            //     break;
            // }
            g_blufi_config.wifi.cwmode = WIFIMODE_STA;
            break;
        case AXK_BLUFI_EVENT_REQ_CONNECT_TO_AP:
        {
            ...
            {
                LOG_I("BLUFI axk_hal_conn_ap_info_set fail\r\n");
                break;
            }
